"""Resonance Proximal Policy Optimization (PPO) Training Script.

This script implements Proximal Policy Optimization for vision-language model alignment.
PPO is an on-policy reinforcement learning method that uses a reward model to optimize
the policy while preventing large policy updates through a clipped objective function.

Key Features:
    - PPO-based reinforcement learning from human feedback (RLHF)
    - Integration with vision-language reward models
    - Value head training for policy optimization
    - Multi-modal response generation and evaluation
    - KL divergence regularization from reference policy
    - Support for LoRA/QLoRA efficient training
    - Automatic generation configuration and sampling

The PPO algorithm alternates between:
1. Generating responses using the current policy
2. Evaluating responses with the reward model  
3. Computing advantages and updating the policy
4. Updating the value function to predict returns

Usage:
    python src/resonance/ppo.py \\
...
from transformers import GPTQConfig, HfArgumentParser
import deepspeed
...
from resonance.utils.common import get_vision_tower, safe_save_model_for_ppo_trainer
from resonance.utils.data import build_dataset_from_vlquery_json
...
class ScriptArguments:
    ...
    image_root: Optional[str] = field(
        default=None, metadata={"help": "path of image root"}
    )
    model_name_or_path: Optional[str] = field(
        default="llava-hf/llava-1.5-7b-hf", metadata={"help": "the model name"}
    )
    reward_adapter: Optional[str] = field(
        default=None, metadata={"help": "the reward adapter path"}
    )
    reward_adapter_name: Optional[str] = field(
        default="reward_adapter", metadata={"help": "the reward adapter name"}
    )
...
class PPOConfig(trl.PPOConfig):
    ...
    project_name: Optional[str] = field(
        default="Resonance", metadata={"help": "wandb project name"}
    )
    ...
    fsdp: str = field(
        default="",
        metadata={
            "help": "Fully Sharded Data Parallelism. This feature has not been implemented yet."
        },
    )
    ...
    output_dir: Optional[str] = field(
        default=None, metadata={"help": "output directory"}
    )
    per_device_gamelog_size: int = field(
        default=2, metadata={"help": "gamelog size per device"}
    )
    max_new_tokens: Optional[int] = field(
        default=None, metadata={"help": "max new tokens for generation"}
    )
    max_length: Optional[int] = field(
        default=None, metadata={"help": "max length for generation"}
    )
    ...
    def __post_init__(self):
        ...
        self.tracker_kwargs = {
            "wandb": {"group": self.group_name, "name": self.run_name}
        }
        ...
        if self.use_value_adapter and not self.use_lora:
            raise ValueError(
                "You can only use value adapter with a Peft base model. Please set use_lora to True."
            )
...
if __name__ == "__main__":
    ...
    if (
        script_args.reward_adapter is not None
        and script_args.reward_model_name_or_path is not None
    ):
    ...
    if script_args.reward_model_name_or_path is not None:
        reward_model = MyAutoRewardModel.from_pretrained(
    ...
    model = MyAutoModelWithValueHead.from_pretrained(
        ...
        value_adapter_config=(
            deepcopy(lora_config) if ppo_config.use_value_adapter else None
        ),
    ...
    dataset = build_dataset_from_vlquery_json(
        local_rank, script_args.data_dir, script_args.image_root
    )
    generation_config = MyAutoGenerationConfig.from_pretrained(
        script_args.model_name_or_path
    )
