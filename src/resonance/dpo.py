"""Resonance Direct Preference Optimization (DPO) Training Script.

This script implements Direct Preference Optimization for vision-language models in Resonance.
DPO is a method for aligning models with human preferences without requiring a separate 
reward model, making it more efficient than traditional RLHF approaches.

Key Features:
    - Direct preference learning from paired comparison data
    - Configurable beta parameter for KL regularization
    - Support for score-based data filtering
    - Multi-modal preference alignment for vision-language tasks
    - LoRA/QLoRA support for efficient training
    - Automatic model and processor loading

The DPO loss function optimizes the model to prefer chosen responses over rejected ones
while staying close to the reference model through KL regularization.

Usage:
    python src/resonance/dpo.py \\
...
class ScriptArguments:
    ...
    data_ratio: Optional[float] = field(
        default=1.0, metadata={"help": "ratio of data to use"}
    )
    image_root: Optional[str] = field(
        default=None, metadata={"help": "path of image root"}
    )
    dataset_name: Optional[str] = field(
        default="vlfeedback_paired", metadata={"help": "name of the dataset"}
    )
...
if __name__ == "__main__":
    ...
    train_dataset = train_dataset.select(
        range(int(len(train_dataset) * script_args.data_ratio))
    )
