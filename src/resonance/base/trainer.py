"""Resonance Vision-Language Trainer Classes.

This module implements specialized trainers for vision-language RLHF training, extending
TRL's trainers with multi-modal capabilities. The trainers handle the complexities of
training with both visual and textual inputs in reinforcement learning scenarios.

Key Components:
    - VLDPOTrainer: Direct Preference Optimization for vision-language models
    - VLPPOTrainer: Proximal Policy Optimization with vision inputs
    - VLSFTTrainer: Supervised Fine-Tuning for instruction following
    - VLRewardTrainer: Training reward models on preference data

Each trainer is optimized for multi-modal inputs and includes specialized data collation,
loss computation, and evaluation metrics appropriate for vision-language tasks.
...
from trl import DPOTrainer, PPOConfig, PPOTrainer, RewardTrainer, SFTTrainer
...
class VLDPOTrainer(DPOTrainer, ABC):
    ...
    def concatenated_inputs(
        ...
        if "img_input_dict" in batch:
            ...
            concatenated_batch["concatenated_img_input_dict"] = (
                concatenated_img_input_dict
            )
...
class VLPPOTrainer(PPOTrainer, ABC):
    ...
    def tokenize_row(self, element):
        ...
        tokens = {
            k: v[0] for k, v in tokens.items() if k != "labels"
    ...
    def train(self):
        ...
        for epoch, batch in enumerate(self.dataloader):
            ...
            convs = [
                self.processor.make_single_turn_conv(q, r)
            ...
            try:
                if self.reward_model is not None:
                    ...
                    rewards = self.accelerator.unwrap_model(
                        self.model
            ...
            batch = {
                k: v[: self.config.per_device_gamelog_size] for k, v in batch.items()
            }
            rewards = rewards[: self.config.per_device_gamelog_size]
            batch["image"] = [wandb.Image(image) for image in batch["image"]]
            batch["query"] = [
                self.processor.remove_image_placeholder(query)
                for query in batch["query"]
            ]
            self.log_stats(
                stats, batch, rewards, columns_to_log=["image", "query", "response"]
            )
            if self.accelerator.is_main_process:
                bar.update()
...
class VLRMTrainer(RewardTrainer, ABC):
    ...
    def tokenize_row(self, element):
        ...
        return {
            ...
            "attention_mask_chosen": chosen_full_tokens["attention_mask"][0][
                : self.max_length
            ],
            "input_ids_rejected": rejected_full_tokens["input_ids"][0][
                : self.max_length
            ],
            "attention_mask_rejected": rejected_full_tokens["attention_mask"][0][
                : self.max_length
            ],
        }
...
class VLSFTTrainer(SFTTrainer, ABC):
    ...
    def tokenize_row(self, element):
        ...
        tokens = self.processor.process_batch_conv([conv])[
            "full"
        ]  # process_batch_conv adds bos token but no eos token
        tokens = {k: v[0] for k, v in tokens.items()}
        # mask eos token
        eos_indices = [
            i
            for i, x in enumerate(tokens["input_ids"])
            if x == self.tokenizer.eos_token_id
        ]
        tokens["attention_mask"] = [
            0 if i in eos_indices else m for i, m in enumerate(tokens["attention_mask"])
        ]
        tokens["labels"] = [
            -100 if i in eos_indices else l for i, l in enumerate(tokens["labels"])
        ]
