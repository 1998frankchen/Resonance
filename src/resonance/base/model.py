"""Resonance Vision-Language Model Base Classes.

This module contains the core model classes for vision-language models used in
Resonance. It provides unified interfaces for reward models and value head models
...
class VLRewardModel(nn.Module, ABC):
    """Vision-Language Reward Model for RLHF training.

    This class wraps a base vision-language model with a reward head for
    reward modeling in RLHF. The reward head outputs scalar rewards for
    input sequences, which are used to train preference-based models.

    Args:
        base_model: Pre-trained vision-language model
        rm_head: Optional reward head layer. If None, creates a linear layer.
    """
    ...
    def __init__(self, base_model: PreTrainedModel, rm_head: nn.Linear | None = None):
        ...
        if rm_head is None:
            # Create default reward head with zero-initialized bias
            rm_head = nn.Linear(hidden_size, 1)
            nn.init.zeros_(rm_head.bias)

        # Move reward head to same device as base model
    ...
    def forward(self, input_ids, attention_mask, *args, **kwargs):
        ...
        Args:
            ...
            *args, **kwargs: Additional arguments for the base model

        Returns:
            Tuple containing reward scores for each sequence
        """
        # Ensure we get hidden states for reward computation
        ...
        # Ensure all parameters are used in backward pass (hack from LLaVA-RLHF)
        last_hidden_state = last_hidden_state + 0.0 * torch.mean(logits)

        # Get final hidden state for reward computation
        last_hidden_state_at_the_end = last_hidden_state[:, -1, :]
        rewards = self.rm_head(last_hidden_state_at_the_end)

        return (rewards,)  # Return tuple for compatibility with RewardTrainer
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        torch.save(
            self.rm_head.state_dict(), os.path.join(save_directory, "rm_head.bin")
        )
    ...
    def _get_reward_head_from_pretrained(
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if value_adapter_config is not None:
            if model.is_peft_model:
                pretrained_model.add_adapter(
                    peft_config=value_adapter_config, adapter_name=value_adapter_name
                )
            else:
                ...
                pretrained_model = get_peft_model(
                    ...
                    adapter_name=value_adapter_name,
                )
