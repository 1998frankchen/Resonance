"""Resonance Reward Model Training Script.

This script trains reward models for vision-language RLHF (Reinforcement Learning from
Human Feedback). The reward model learns to predict human preferences by training on 
paired comparison data, where each pair contains a chosen and rejected response.

Key Features:
    - Bradley-Terry preference modeling for paired comparisons
    - Vision-language reward prediction for multi-modal responses  
    - Support for score-based and preference-based training data
    - LoRA/QLoRA for efficient training of large models
    - Configurable score margins for data filtering
    - Integration with PPO training pipeline

The reward model is trained using a ranking loss that maximizes the score difference
between chosen and rejected responses. The trained model outputs scalar rewards that
guide policy optimization in PPO training.

Training Process:
1. Load paired comparison data (chosen vs rejected responses)
2. Filter pairs based on score margins if provided
3. Train model to predict preference rankings
4. Evaluate on held-out preference data
5. Save trained reward model for PPO

Usage:
    python src/resonance/reward_modeling.py \\
        --model_name_or_path path/to/sft/model \\
        --dataset_name vlfeedback_paired \\
        --data_path path/to/preference/data.json \\
...
from resonance.utils.common import get_vision_tower, safe_save_model_for_hf_trainer
from resonance.utils.data import make_vlfeedback_paired_dataset
...
if __name__ == "__main__":
    ...
    dataset = make_vlfeedback_paired_dataset(
        local_rank, script_args.data_dir, script_args.score_margin
    )
