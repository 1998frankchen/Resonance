"""Resonance Data Processing Utilities.

This module contains functions for loading, processing, and formatting datasets used
in vision-language RLHF training. It handles various dataset formats including
preference pairs, instruction tuning data, and evaluation datasets.

Key Functions:
    - make_vlfeedback_paired_dataset: Creates preference pairs from VL-Feedback
    - build_dataset_from_vlquery_json: Loads instruction datasets from JSON
    - DATASET_MAP: Registry of supported dataset types and their loaders

The module supports multiple data formats:
    - Paired preference data for DPO/reward modeling
    - Instruction-response data for SFT
    - Multi-modal conversation data with images
...
def make_vlfeedback_paired_dataset(script_args):
    ...
    def make_batch_pairs(sample):
        ...
        for sample_idx, comps in enumerate(sample["completions"]):
            ...
            for comp_idx1, comp_idx2 in combinations(
                range(len(comps["annotations"])), 2
            ):
                anno1, anno2 = (
                ...
                try:
                    avg_score1 = np.mean(
                        [float(anno1[aspect]["Rating"]) for aspect in anno1]
                    )
                    avg_score2 = np.mean(
                        [float(anno2[aspect]["Rating"]) for aspect in anno2]
                    )
                except ValueError:
                    continue
    ...
    with PartialState().local_main_process_first():
        ds = ds.map(
            ...
            - set(["prompt", "chosen", "rejected", "img_path"]),
            keep_in_memory=True,
        )
...
def make_rlhfv_paired_dataset(script_args):
    ds = load_dataset(
        "HaoyeZhang/RLHF-V-Dataset", split="train", trust_remote_code=True
    )
    ...
    def gen():
        for d in raw_data:
            ...
            if "image" in d:
                ...
                img_path = None
            yield dict(
                img_path=img_path, prompt=prompt, chosen=chosen, rejected=rejected
            )
