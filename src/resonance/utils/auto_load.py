"""Resonance Auto-Loading Utilities.

This module provides automatic loading and configuration utilities for vision-language
models, trainers, and data collators. It abstracts away model-specific implementation
details and provides a unified interface for loading different model architectures.

Key Features:
    - Automatic model detection and loading based on model names
    - Unified processor creation for different vision-language models
    - Factory functions for trainers and data collators
    - Support for LoRA, QLoRA, and quantization configurations
    - Integration with DeepSpeed and distributed training

The auto-loading system supports popular vision-language models including:
    - LLaVA variants (LLaVA-1.5, LLaVA-Next)
    - Qwen-VL models
    - InstructBLIP models
    - InternLM-XComposer2 models
...
def auto_core_mapper(architecture):
    ...
    return import_module(module_name, package="resonance.models").core_mapper
        ...
        if os.path.exists(adapter_path):
            ...
            if kwargs.get("use_flash_attention_2", False):
                if architecture not in FLASH_ATTN_MODELS:
                    logger.warning(
                        f"{architecture} does not support flash attention 2. Disabling."
                    )
...
class MyAutoProcessor:
    ...
    def from_pretrained(model_name_or_path, **kwargs) -> VLProcessor:
        config = AutoConfig.from_pretrained(model_name_or_path, trust_remote_code=True)
        architecture = config.architectures[0]
        return auto_core_mapper(architecture).processor(
...
# Factory function aliases for backwards compatibility
MyAutoSFTCollator = VLSFTDataCollatorWithPadding
MyAutoDPOCollator = VLDPODataCollatorWithPadding
MyAutoRMCollator = VLRMDataCollatorWithPadding
MyAutoPPOCollator = VLPPODataCollator
MyAutoProcessor = VLProcessor
MyAutoSFTTrainer = VLSFTTrainer
MyAutoDPOTrainer = VLDPOTrainer
MyAutoRMTrainer = VLRMTrainer
MyAutoPPOTrainer = VLPPOTrainer
MyAutoModelWithValueHead = VLModelWithValueHead
MyAutoRewardModel = VLRewardModel


class MyAutoGenerationConfig:
    """Auto generation config factory for different models."""

    @classmethod
    def from_pretrained(cls, model_name_or_path, **kwargs):
        """Load generation config from pretrained model."""
        return GenerationConfig.from_pretrained(model_name_or_path, **kwargs)
