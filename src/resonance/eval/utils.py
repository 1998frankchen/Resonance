"""Resonance Evaluation Utilities and Base Classes.

This module provides core utilities and abstract base classes for evaluation
benchmarks in the Resonance framework. It standardizes the evaluation process
across different vision-language benchmarks and provides common functionality
for data loading, result calculation, and database storage.

🎯 **Core Features:**
- Abstract evaluation base class for consistent benchmark implementation
- Database integration for result storage and tracking
- Batch processing utilities for efficient evaluation
- Multi-modal data loading and preprocessing
- Result aggregation and metric calculation
- Memory management for large-scale evaluations

🏗️ **Architecture:**
- AbstractEvaluator: Base class for all benchmark evaluations
- Database utilities for MySQL result storage
- Batch processing optimizations
- GPU memory management helpers
- Result formatting and export functions

📊 **Supported Operations:**
- Model inference with generation configuration
- Batch-wise evaluation for memory efficiency
- Result caching and persistence
- Metric calculation and aggregation
- Cross-benchmark result comparison
...
def run_vqa(
    ...
    dataloader = DataLoader(
        dataset=dataset, batch_size=batch_size, collate_fn=collator(processor)
    )
    ...
    with torch.inference_mode():
        for inputs, others in dataloader:
            ...
            if outputs.shape[1] > input_token_len and torch.all(
                outputs[:, :input_token_len] == inputs["input_ids"]
            ):
            ...
            results.extend(
                [
                    other
                    for other, res in zip(others, responses)
                    if other.update(response=res.strip()) is None
                ]
            )
    ...
    with torch.inference_mode():
        for inputs, others in dataloader:
            ...
            results.extend(
                [
                    other
                    for other, res in zip(others, ce_losses)
                    if other.update(ppl=res) is None
                ]
            )
...
def run_vqa_sgl(
    ...
    results.extend(
        [
            other
            for other, res in zip(all_others, states)
            if other.update(response=res["answer"].strip()) is None
        ]
    )
    return results
...
def log_data_to_mysql(host, port, user, passward, database, table, data_dict):
    db = pymysql.connect(
        host=host, user=user, passwd=passward, database=database, port=port
    )
    cursor = db.cursor()
