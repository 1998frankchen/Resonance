...
class Collator(VLCollator):
    def __call__(self, batch):
        ...
        prompts = [
            ...
            for q, img in zip(prompts, images)
        ]
        convs = [
            self.processor.make_single_turn_conv(prompt, res)
            for prompt, res in zip(prompts, responses)
        ]
        inputs = self.processor(convs=convs, images_path=images, padding_side="right")
        others = [
            dict(question_id=id, choice=choice)
            for id, choice in zip(question_ids, choices)
        ]
...
if __name__ == "__main__":
    ...
    results = run_vqa_ppl(
        model_path, dataset, Collator, accelerator, args.processor_path, args.batch_size
    )
    final_answers = {}
    for r in results:
