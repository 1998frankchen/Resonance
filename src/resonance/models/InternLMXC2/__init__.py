    ...
    def prepare_default_generation_kwargs(self, generation_config):
        ...
        class KeywordsStoppingCriteria(StoppingCriteria):
            ...
            def __call__(self, input_ids: torch.LongTensor, *args, **kwargs):
                ...
                input_ids[:, -1] = (
                    input_ids[:, -1] * (~is_done) + self.pad_token_id * is_done
                )
                return is_done

        stopping_criteria_list = StoppingCriteriaList(
            [KeywordsStoppingCriteria([92542], 2)]
        )  # [UNUSED_TOKEN_145]
        kwargs = dict(
...
class InternLMXC2RewardModel(VLRewardModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        base_model = InternLMXComposer2ForCausalLM.from_pretrained(
        ...
        for source in sources:
            ...
            for idx, sentence in enumerate(source):
                ...
                full_tokens = self.tokenizer(
                    extend_text, padding=False, add_special_tokens=(idx == 0)
                )
                ...
                if (
                    ...
                    if extend_len < len(full_tokens["input_ids"]):
                        ...
                        labels[-len(text_tokens["input_ids"]) :] = text_tokens[
                            "input_ids"
                        ]
            labels = [
    ...
    def __call__(
        ...
        if images_path is not None:
            images = [
                ...
                for img_path in flatten_list(images_path)
            ]
...
class InternLMXC2DPODataCollatorWithPadding(VLDPODataCollatorWithPadding):
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        padded_batch["img_input_dict"] = dict(
            pixel_values=self.processor.image_processor(images=imgs)
        )
        return padded_batch
