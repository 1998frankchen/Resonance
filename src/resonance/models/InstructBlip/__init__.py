...
from resonance.base.model import VLModelWithValueHead, VLRewardModel
...
class InstructBlipForRL(InstructBlipForConditionalGeneration):
    def forward(
        ...
        img_pos = torch.full(
            output.language_model_outputs.logits.shape[:2], False, dtype=torch.bool
        )
        img_pos[:, :img_token_num] = True
        attentions = (
            output.language_model_outputs.attentions if output_attentions else None
        )
...
class InstructBlipRewardModel(VLRewardModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        base_model = InstructBlipForRL.from_pretrained(
...
class InstructBlipProcessor(VLProcessor):
    def __init__(self, model_name_or_path, **kwargs) -> None:
        self.processor = transformers.InstructBlipProcessor.from_pretrained(
    ...
    def process_batch_conv(
        ...
        if not isinstance(sources, list) or not isinstance(sources[0], list):
            raise ValueError(
                "sources must be a batch of conversations, eg. List[List[Dict]]"
            )
        role_begin = {
            "user": self.chat_template.user_begin,
            "assistant": self.chat_template.assistant_begin,
        }
        role_end = {
            "user": self.chat_template.user_end,
            "assistant": self.chat_template.assistant_end,
        }
        raw_texts = []
        batch_input_ids = []
        ...
        for source in sources:
            ...
            if len(source) > 2:
                raise ValueError(
                    "InstructBlip does not support multi-turn conversation."
                )
            for idx, sentence in enumerate(source):
                ...
                text_tokens = self.tokenizer(
                    sentence["value"], padding=False, add_special_tokens=(idx == 0)
                )
                ...
                if (
                    sentence["from"] == "assistant"
                    and len(text_tokens["input_ids"]) != 0
                ):
                    target_len = max(
                        min([extend_len, len(text_tokens["input_ids"]), len(labels)]), 1
                    )
    ...
    def __call__(
        ...
        inputs = super().__call__(
            texts, convs, images_path, padding, padding_side, check_format
        )
        if convs and texts is None:
            texts = [conv[0]["value"] for conv in convs]
        qformer_inputs = self.qformer_tokenizer(
            text=texts, padding=padding, return_tensors="pt"
        )
...
class InstructBlipDPODataCollatorWithPadding(VLDPODataCollatorWithPadding):
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        imgs = [
            Image.open(img_path).convert("RGB") for img_path in padded_batch["img_path"]
        ]
        img_input_dict = {}
        img_input_dict["pixel_values"] = self.processor.image_processor(
        ...
        qformer_pad_id = (
            torch.ones_like(padded_qformer_input_ids) * qformer_tokenizer.pad_token_id
        )
        padded_qformer_input_ids = torch.where(
            padded_qformer_input_ids > len(qformer_tokenizer) - 1,
            qformer_pad_id,
            padded_qformer_input_ids,
        )
        padded_qformer_input_ids = torch.where(
            padded_qformer_input_ids == self.pad_token_id,
            qformer_pad_id,
            padded_qformer_input_ids,
        )
        img_input_dict["qformer_input_ids"] = padded_qformer_input_ids
        img_input_dict["qformer_attention_mask"] = padded_batch.pop(
            "qformer_attention_mask"
        )
...
class InstructBlipSFTDataCollatorWithPadding(VLSFTDataCollatorWithPadding):
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        padded_batch["pixel_values"] = self.processor.image_processor(
            images=imgs, return_tensors="pt"
        )["pixel_values"]
...
class InstructBlipRMTrainer(VLRMTrainer):
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            "Reward model is not supported for InstructBlip currently."
        )
